...
import { addSystem, updateSystem, toggleSystemStatus, renewSystem } from "@/app/actions";
...
export default function Dashboard({
  ...
  const [renewingSystem, setRenewingSystem] = useState<System | null>(null);
  ...
  const openEditModal = (sys: System) => {
    ...
    setCustomerPhone(sys.customerPhone || "");
    setAccountantPhone(sys.accountantPhone || "");
    setModalOpen(true);
  };

  const openRenewModal = (sys: System) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto font-sans" dir="rtl">
            ...
            <div key={sys.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-shadow">
              ...
              <div className="space-y-3 mb-6">
                ...
                {sys.customerPhone && (
                  <div className="flex justify-between text-sm">
                    ...
                    <a href={`tel:${sys.customerPhone}`} className="font-semibold text-blue-600" dir="ltr">{sys.customerPhone}</a>
                  </div>
                )}
                {sys.accountantPhone && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">هاتف المحاسب:</span>
                    <a href={`tel:${sys.accountantPhone}`} className="font-semibold text-blue-600" dir="ltr">{sys.accountantPhone}</a>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-50">
                {isOwner ? (
                  ...
                  <button
                    onClick={() => openRenewModal(sys)}
                    className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-700 py-2 rounded-lg text-sm font-semibold transition-colors"
                  >
                    تجديد الاشتراك
                  </button>
                )}
      ...
      {/* Full edit/add modal — owner only */}
      {isModalOpen && (
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="tel" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="01xxxxxxxxx" dir="ltr" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">هاتف المحاسب (إن وجد)</label>
                <input type="tel" value={accountantPhone} onChange={e => setAccountantPhone(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="01xxxxxxxxx" dir="ltr" />
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-500 mt-1">{renewingSystem.displayName}</p>
            </div>
            ...
            <form onSubmit={handleRenewSubmit} className="p-6 space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none p-3 bg-green-50 rounded-lg border border-green-100">
                <input
                  ...
                  onChange={e => setRenewRecordPayment(e.target.checked)}
                  className="w-4 h-4 accent-green-600"
                />
                <span className="text-sm font-semibold text-green-800">تسجيل دفعة عند التجديد</span>
              </label>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="button" onClick={() => setRenewingSystem(null)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-2.5 rounded-lg font-semibold transition-colors">
                  إلغاء
                </button>
              </div>
            </form>
